-- ============================================================================
-- CREATE DEFAULT ORGANIZATION FOR STANDALONE PARENTS (FIXED)
-- ============================================================================
-- This creates "EduDash Pro - Standalone Parents" as a virtual organization
-- for users who haven't joined a real school yet.
-- ============================================================================

-- Step 1: Add is_default_standalone column if it doesn't exist
ALTER TABLE preschools 
ADD COLUMN IF NOT EXISTS is_default_standalone BOOLEAN DEFAULT FALSE;

-- Step 2: Create the default standalone organization
INSERT INTO preschools (
    ...
    'e7d4c3b2-a1f0-4e5d-9c8b-7a6f5e4d3c2b'::uuid,  -- Fixed UUID for reference
    'EduDash Pro - Standalone Parents',
    'support@edudashpro.org.za',
    '+27 10 123 4567',
    'Virtual Community',
    true,
    true,  -- Mark as the default standalone org
    'completed',
    true,
    NOW(),
    NOW()
...
-- Step 4: Create helper function to get default standalone org ID
...
-- Step 5: Update the profile creation trigger to auto-assign standalone org
...
BEGIN
    ...
    -- Get default standalone org ID (only for parents without organization)
    ...
    INSERT INTO public.profiles (
        ...
        user_last_name,
        user_phone,
        default_org_id,  -- Auto-assign to default org for parents
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = NOW(),
        -- Assign to standalone org if they don't have one
        preschool_id = CASE 
...
-- Step 7: Migrate existing standalone parents to the default org
...
WHERE 
    ...
    AND created_at > NOW() - INTERVAL '90 days';  -- Only recent users (last 3 months)

-- Step 8: Grant trial to existing users who don't have one
UPDATE public.profiles
SET 
    is_trial = TRUE,
...
-- Step 9: Create function to transfer user to real school
...
BEGIN
    -- Get current school
    SELECT preschool_id INTO old_school_id
    FROM profiles
    WHERE id = user_id;
    
    -- Get default standalone org
    default_org_id := get_default_standalone_org_id();
...
-- Step 10: Create function to move user back to standalone
...
SELECT 
    'Default organization created' as status,
    id,
    name,
    is_default_standalone
FROM preschools
...
SELECT 
    'Standalone parents migrated' as status,
    COUNT(*) as count
FROM profiles
WHERE preschool_id = get_default_standalone_org_id();

SELECT 
    'Users granted trial' as status,
    COUNT(*) as count
FROM profiles
WHERE is_trial = TRUE AND trial_started_at > NOW() - INTERVAL '2 minutes';

-- ============================================================================
-- DONE!
-- ============================================================================
SELECT '✅ Default standalone organization created and users migrated!' as result;
